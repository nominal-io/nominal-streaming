...
pub struct StreamRuntime {
    ...
    /// called `enqueue`.
    ///
    /// Owned rather than shared: dropping it is what drains the stream, so a stray clone would turn
    /// that drain into a silent no-op. Note also that it must drop before `shutdown_tx` -- the
    /// uploader runs its final requests on the runtime that sender shuts down -- which is why
    /// `close` takes both apart explicitly rather than relying on field order.
