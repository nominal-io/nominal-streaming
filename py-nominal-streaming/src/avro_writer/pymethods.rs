...
use crate::point::extract_series_points;
...
impl NominalAvroWriter {
    ...
    fn write_batch(
        ...
        let descriptor = description_with_tags(channel, tags);
        let points = extract_series_points(extract_vec_ts(ts_ns), &values)?;
        ...
        match points {
            PointsType::DoublePoints(points) => py.detach(|| {
                self.inner
                    .write(&descriptor, points.points)
                    .map_err(map_err)
            }),
            PointsType::IntegerPoints(points) => py.detach(|| {
                self.inner
                    .write(&descriptor, points.points)
                    .map_err(map_err)
            }),
            PointsType::StringPoints(points) => py.detach(|| {
                self.inner
                    .write(&descriptor, points.points)
                    .map_err(map_err)
            }),
            _ => unreachable!("extract_series_points returns only scalar series"),
        }
    }
