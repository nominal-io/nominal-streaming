...
| [validate.yml](validate.yml) | Code validation: Rust and Python formatting, linting, builds, and tests | PR opened, reopened, or synchronized; pushes to main |
...
Rust builds and tests share a job; Python linting and type checks share a job.
...
`rust-lint`, `rust-build-test`, `python-format`, and `python-lint`. Omit redundant
job names so the ID is also the required-check context.
